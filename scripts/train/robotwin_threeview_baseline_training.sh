...
ROBOTWIN_DATASET_ROOT="${ROBOTWIN_DATASET_ROOT:-/meta_eon_cfs/home/lqc/dataset/robotwin/dreamzero_robotwin_baseline}"  # 要改成自己的实际路径
ROBOTWIN_DEPTH_ROOT="${ROBOTWIN_DEPTH_ROOT:-$ROBOTWIN_DATASET_ROOT}"
ROBOTWIN_OUTPUT_NAME="${ROBOTWIN_OUTPUT_NAME:-}"
OUTPUT_DIR="${OUTPUT_DIR:-$DREAMZERO_ROOT/checkpoints/robotwin_threeview_baseline_50train_full}"
NUM_GPUS="${NUM_GPUS:-8}"
PER_DEVICE_BS="${PER_DEVICE_BS:-1}"  # 没有 150G 显存只能用 bs=1
GLOBAL_BATCH_SIZE="${GLOBAL_BATCH_SIZE:-$((NUM_GPUS * PER_DEVICE_BS))}"
MAX_STEPS="${MAX_STEPS:-120000}"  # 50-task 最好大于 100k
SAVE_STEPS="${SAVE_STEPS:-5000}"
LEARNING_RATE="${LEARNING_RATE:-1e-5}"
DEEPSPEED_CFG="${DEEPSPEED_CFG:-zero2}"

# Bucket batching lets PER_DEVICE_BS>1 work with RoboTwin samples that have different natural chunk counts.
# bs>1 才开分桶
...
if [ "${ROBOTWIN_TASKS:-all}" = "all" ]; then
    ROBOTWIN_TASK_ARRAY=()
    if [ -d "$ROBOTWIN_DATASET_ROOT" ]; then
        while IFS= read -r task_root; do
            if [ -f "$task_root/meta/modality.json" ]; then
                ROBOTWIN_TASK_ARRAY+=("$(basename "$task_root")")
            fi
        done < <(find "$ROBOTWIN_DATASET_ROOT" -mindepth 1 -maxdepth 1 -type d | sort)
    fi
...
if [ "${#ROBOTWIN_TASK_ARRAY[@]}" -eq 0 ]; then
    echo "ERROR: no converted RoboTwin datasets found under $ROBOTWIN_DATASET_ROOT"
    echo "Build them with scripts/data/convert_robotwin_threeview_tasks.sh"
    exit 1
fi
...
if [ -n "${ROBOTWIN_DATA_ROOTS:-}" ]; then
    ...
    for task in "${ROBOTWIN_TASK_ARRAY[@]}"; do
        task_root="$ROBOTWIN_DATASET_ROOT/$task"
        if [ ! -d "$task_root" ]; then
            echo "ERROR: DreamBase RoboTwin dataset not found for task '$task': $task_root"
            echo "Build it with scripts/data/convert_robotwin_threeview_tasks.sh"
            exit 1
        fi
        if [ ! -f "$task_root/meta/modality.json" ]; then
...
echo "RoboTwin dataset root: $ROBOTWIN_DATASET_ROOT"
