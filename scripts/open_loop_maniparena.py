#!/usr/bin/env python3
"""Offline open-loop evaluation for DreamZero on ManipArena EE data.

This follows scripts/open_loop_yam.py closely:
- load a DreamZero checkpoint directly;
- read LeRobot-style parquet + MP4 files;
- run one inference per sampled timestep;
- compare the first predicted action step with the ground-truth action.

Usage:
    conda activate /meta_eon_cfs/home/lqc/miniconda3/envs/dreamzero
    cd /meta_eon_cfs/home/Robochallenge/ma/lqc/dreamzero
    NO_ALBUMENTATIONS_UPDATE=1 TORCHDYNAMO_DISABLE=1 CUDA_VISIBLE_DEVICES=0 \
    python scripts/open_loop_maniparena.py \
        --model_path checkpoints/maniparena_preliminary_debug/checkpoint-4500 \
        --dataset_path data/dreamzero_maniparena_preliminary_ee/press_button_in_order \
        --device cuda:0 \
        --use_dataset_prompt \
        --num_samples 200 \
        --output_dir openloop_results
"""

import argparse
import glob
import os
import sys
import time
from pathlib import Path

import cv2
import matplotlib

matplotlib.use("Agg")
import matplotlib.pyplot as plt
import numpy as np
import pyarrow.parquet as pq
import torch
import torch.distributed as dist
from tianshou.data import Batch

# 确保优先使用当前 DreamZero 仓库里的 groot，而不是 conda 环境中可能存在的旧包。
DREAMZERO_ROOT = Path(__file__).resolve().parents[1]
if str(DREAMZERO_ROOT) not in sys.path:
    sys.path.insert(0, str(DREAMZERO_ROOT))

from groot.vla.data.schema import EmbodimentTag
from groot.vla.model.n1_5.sim_policy import GrootSimPolicy


# ---------------------------------------------------------------------------
# ManipArena EE layout
# ---------------------------------------------------------------------------

VIDEO_CAMERAS = {
    "video.faceImg": "observation.images.faceImg",
    "video.leftImg": "observation.images.leftImg",
    "video.rightImg": "observation.images.rightImg",
}

STATE_SLICES = {
    "state.left_ee_pos": (0, 6),
    "state.left_gripper_pos": (6, 7),
    "state.right_ee_pos": (7, 13),
    "state.right_gripper_pos": (13, 14),
}

ACTION_SLICES = {
    "action.left_ee_pos": (0, 6),
    "action.left_gripper_pos": (6, 7),
    "action.right_ee_pos": (7, 13),
    "action.right_gripper_pos": (13, 14),
}

ACTION_KEY_ORDER = [
    "action.left_ee_pos",
    "action.left_gripper_pos",
    "action.right_ee_pos",
    "action.right_gripper_pos",
]

ACTION_WIDTHS = {
    "action.left_ee_pos": 6,
    "action.left_gripper_pos": 1,
    "action.right_ee_pos": 6,
    "action.right_gripper_pos": 1,
}


# ---------------------------------------------------------------------------
# Dataset reader  (LeRobot chunked format)
# ---------------------------------------------------------------------------

class ManipArenaDataset:
    """Reads ManipArena EE LeRobot-style chunked parquet + MP4."""

    def __init__(self, dataset_path: str):
        self.root = dataset_path

        data_dir = os.path.join(dataset_path, "data")
        parquet_files = sorted(glob.glob(os.path.join(data_dir, "**", "episode_*.parquet"), recursive=True))
        if not parquet_files:
            raise FileNotFoundError(f"No episode_*.parquet found under {data_dir}")

        self.episodes = []
        self.episode_ids = []
        self.cum_lengths = [0]
        for pf in parquet_files:
            table = pq.read_table(pf)
            episode_id = int(os.path.basename(pf).removeprefix("episode_").removesuffix(".parquet"))
            self.episodes.append(table)
            self.episode_ids.append(episode_id)
            self.cum_lengths.append(self.cum_lengths[-1] + table.num_rows)
        self.total_rows = self.cum_lengths[-1]

        videos_root = os.path.join(dataset_path, "videos")
        self.video_dirs = {}
        for server_key, folder_name in VIDEO_CAMERAS.items():
            candidates = sorted(glob.glob(os.path.join(videos_root, "**", folder_name), recursive=True))
            if candidates:
                self.video_dirs[server_key] = candidates[0]
        missing = [key for key in VIDEO_CAMERAS if key not in self.video_dirs]
        if missing:
            raise FileNotFoundError(f"Missing video folders for: {missing}")

        print(
            f"ManipArenaDataset: {len(self.episodes)} episodes, "
            f"{self.total_rows} rows, {len(self.video_dirs)} cameras"
        )

    def __len__(self):
        return self.total_rows

    def _locate(self, idx):
        for ep in range(len(self.episodes)):
            if idx < self.cum_lengths[ep + 1]:
                return ep, idx - self.cum_lengths[ep]
        raise IndexError(f"Index {idx} out of range ({self.total_rows})")

    def get_state(self, idx) -> np.ndarray:
        ep, row = self._locate(idx)
        return np.array(self.episodes[ep].column("observation.state")[row].as_py(), dtype=np.float64)

    def get_action(self, idx) -> np.ndarray:
        ep, row = self._locate(idx)
        return np.array(self.episodes[ep].column("action")[row].as_py(), dtype=np.float64)

    def get_task(self, idx) -> str:
        ep, row = self._locate(idx)
        try:
            return str(self.episodes[ep].column("annotation.language.action_text")[row].as_py())
        except Exception:
            return ""

    def get_frame(self, idx, server_key) -> np.ndarray:
        """Read one video frame -> (H, W, 3) uint8 RGB."""
        ep, row = self._locate(idx)
        episode_id = self.episode_ids[ep]
        mp4 = os.path.join(self.video_dirs[server_key], f"episode_{episode_id:06d}.mp4")
        cap = cv2.VideoCapture(mp4)
        cap.set(cv2.CAP_PROP_POS_FRAMES, row)
        ret, frame = cap.read()
        cap.release()
        if not ret:
            raise RuntimeError(f"Failed to read frame {row} from {mp4}")
        return cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)


# ---------------------------------------------------------------------------
# Observation builder
# ---------------------------------------------------------------------------

def _validate_ee_vector(vector: np.ndarray, name: str) -> None:
    if vector.shape[0] < 14:
        raise ValueError(f"{name} requires >=14D ManipArena EE vector, got {vector.shape[0]}")


def build_obs(dataset: ManipArenaDataset, idx: int, prompt: str) -> dict:
    """Build an obs dict matching what GrootSimPolicy.forward() expects."""
    obs = {}

    for server_key in dataset.video_dirs:
        frame = dataset.get_frame(idx, server_key)
        obs[server_key] = frame[np.newaxis, ...].astype(np.uint8)

    state = dataset.get_state(idx)
    _validate_ee_vector(state, "state")
    for key, (start, end) in STATE_SLICES.items():
        obs[key] = state[start:end].reshape(1, -1).astype(np.float64)

    obs["annotation.language.action_text"] = prompt
    return obs


def get_gt_action_dict(dataset: ManipArenaDataset, idx: int) -> dict:
    """Split the flat GT action vector into per-key arrays."""
    action_flat = dataset.get_action(idx)
    _validate_ee_vector(action_flat, "action")
    gt = {}
    for key in ACTION_KEY_ORDER:
        start, end = ACTION_SLICES[key]
        gt[key] = action_flat[start:end]
    return gt


def first_pred_step(value, width: int, key: str) -> np.ndarray:
    """Extract the first action step from a model output chunk."""
    if isinstance(value, torch.Tensor):
        value = value.detach().cpu().numpy()
    arr = np.asarray(value, dtype=np.float32)
    while arr.ndim >= 3 and arr.shape[0] == 1:
        arr = arr[0]
    if arr.ndim == 2:
        arr = arr[0]
    elif arr.ndim == 1 and arr.size != width:
        arr = arr[:width]
    arr = arr.reshape(-1)
    if arr.size != width:
        raise ValueError(f"{key} expected first-step width {width}, got shape {np.asarray(value).shape}")
    return arr


# ---------------------------------------------------------------------------
# Plotting
# ---------------------------------------------------------------------------

def save_plots(all_preds, all_gts, key_names, output_dir):
    """Plot pred vs gt for each action dimension across all keys."""
    pred_flat = np.concatenate([all_preds[k] for k in key_names], axis=-1)
    gt_flat = np.concatenate([all_gts[k] for k in key_names], axis=-1)
    dim = pred_flat.shape[1]
    mse_dim = np.mean((pred_flat - gt_flat) ** 2, axis=0)

    for d in range(dim):
        plt.figure(figsize=(10, 4))
        plt.plot(gt_flat[:, d], label="gt", alpha=0.8)
        plt.plot(pred_flat[:, d], label="pred", alpha=0.8)
        plt.title(f"Action dim {d}  (MSE={mse_dim[d]:.6f})")
        plt.xlabel("sample index")
        plt.ylabel("value")
        plt.legend()
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig(os.path.join(output_dir, f"action_dim_{d}.png"), dpi=150)
        plt.close()

    ncols = 4
    nrows = (dim + ncols - 1) // ncols
    fig, axes = plt.subplots(nrows, ncols, figsize=(5 * ncols, 3.5 * nrows), squeeze=False)
    overall_mse = float(np.mean(mse_dim))
    fig.suptitle(f"All action dims  (overall MSE={overall_mse:.6f})", fontsize=14)
    for d in range(dim):
        ax = axes[d // ncols][d % ncols]
        ax.plot(gt_flat[:, d], label="gt", alpha=0.7, lw=0.8)
        ax.plot(pred_flat[:, d], label="pred", alpha=0.7, lw=0.8)
        ax.set_title(f"dim {d} (MSE={mse_dim[d]:.4f})", fontsize=9)
        ax.tick_params(labelsize=7)
        ax.grid(True, alpha=0.2)
        if d == 0:
            ax.legend(fontsize=7)
    for d in range(dim, nrows * ncols):
        axes[d // ncols][d % ncols].set_visible(False)
    fig.tight_layout(rect=[0, 0, 1, 0.96])
    fig.savefig(os.path.join(output_dir, "all_action_dims.png"), dpi=200)
    plt.close(fig)

    fig2, axes2 = plt.subplots(1, len(key_names), figsize=(5 * len(key_names), 4), squeeze=False)
    for i, key in enumerate(key_names):
        ax = axes2[0][i]
        pred, gt = all_preds[key], all_gts[key]
        for d in range(pred.shape[1]):
            ax.plot(gt[:, d], "--", alpha=0.5, lw=0.8)
            ax.plot(pred[:, d], alpha=0.7, lw=0.8)
        key_mse = float(np.mean((pred - gt) ** 2))
        ax.set_title(f"{key}\nMSE={key_mse:.6f}", fontsize=9)
        ax.grid(True, alpha=0.2)
        ax.tick_params(labelsize=7)
    fig2.suptitle("Per-key pred (solid) vs gt (dashed)", fontsize=12)
    fig2.tight_layout(rect=[0, 0, 1, 0.94])
    fig2.savefig(os.path.join(output_dir, "per_key_summary.png"), dpi=200)
    plt.close(fig2)

    return mse_dim, overall_mse


# ---------------------------------------------------------------------------
# Main evaluation
# ---------------------------------------------------------------------------

def evaluate(args):
    # Single-process distributed init (GrootSimPolicy uses dist.get_rank()).
    if not dist.is_initialized():
        os.environ.setdefault("MASTER_ADDR", "localhost")
        os.environ.setdefault("MASTER_PORT", "29500")
        dist.init_process_group(backend="gloo", world_size=1, rank=0)

    print(f"Loading model from {args.model_path} ...")
    policy = GrootSimPolicy(
        embodiment_tag=EmbodimentTag.MANIPARENA_PRELIMINARY_EE,
        model_path=args.model_path,
        device=args.device,
    )
    policy.eval()
    print("Model loaded.")

    dataset = ManipArenaDataset(args.dataset_path)
    os.makedirs(args.output_dir, exist_ok=True)

    num = min(args.num_samples, len(dataset) - args.start_idx)
    preds_per_key = {k: [] for k in ACTION_KEY_ORDER}
    gts_per_key = {k: [] for k in ACTION_KEY_ORDER}
    times = []

    print(f"\nEvaluating {num} samples (start={args.start_idx}) ...")
    print("-" * 60)

    for i in range(num):
        idx = args.start_idx + i

        prompt = args.prompt
        if args.use_dataset_prompt:
            task = dataset.get_task(idx)
            if task:
                prompt = task

        obs = build_obs(dataset, idx, prompt)

        t0 = time.perf_counter()
        with torch.inference_mode():
            result, _ = policy.lazy_joint_forward_causal(Batch(obs=obs))
        elapsed = time.perf_counter() - t0
        times.append(elapsed)

        gt = get_gt_action_dict(dataset, idx)

        for key in ACTION_KEY_ORDER:
            if key in result.act:
                pred_val = first_pred_step(result.act[key], ACTION_WIDTHS[key], key)
                preds_per_key[key].append(pred_val)
                gts_per_key[key].append(gt[key])

        if i % args.log_every == 0:
            if i == 0:
                print(f"  Action keys in output: {list(result.act.keys())}")
                for key in ACTION_KEY_ORDER:
                    if key in result.act:
                        value = result.act[key]
                        shape = value.shape if hasattr(value, "shape") else "?"
                        print(f"    {key}: pred_shape={shape}, gt_shape={gt[key].shape}")
            print(f"  [{i:>5d}/{num}] idx={idx} infer={elapsed:.3f}s prompt={prompt!r:.60}")

    valid_keys = [k for k in ACTION_KEY_ORDER if len(preds_per_key[k]) > 0]
    if not valid_keys:
        print("No predictions!")
        return

    stacked_preds = {k: np.stack(preds_per_key[k]) for k in valid_keys}
    stacked_gts = {k: np.stack(gts_per_key[k]) for k in valid_keys}

    pred_all = np.concatenate([stacked_preds[k] for k in valid_keys], axis=-1)
    gt_all = np.concatenate([stacked_gts[k] for k in valid_keys], axis=-1)
    overall_mse = float(np.mean((pred_all - gt_all) ** 2))

    print(f"\n{'=' * 60}")
    print(f"Overall MSE: {overall_mse:.6f}  |  Avg inference time: {np.mean(times):.4f}s")
    for key in valid_keys:
        key_mse = float(np.mean((stacked_preds[key] - stacked_gts[key]) ** 2))
        print(f"  {key}: MSE={key_mse:.6f}")
    print(f"{'=' * 60}")

    mse_dim, _ = save_plots(stacked_preds, stacked_gts, valid_keys, args.output_dir)

    np.savez(
        os.path.join(args.output_dir, "open_loop_results.npz"),
        pred=pred_all,
        gt=gt_all,
        overall_mse=overall_mse,
        model_path=args.model_path,
        dataset_path=args.dataset_path,
        start_idx=args.start_idx,
        num_samples=num,
    )
    with open(os.path.join(args.output_dir, "mse.txt"), "w", encoding="utf-8") as f:
        f.write(f"overall_mse,{overall_mse}\n")
        for key in valid_keys:
            key_mse = float(np.mean((stacked_preds[key] - stacked_gts[key]) ** 2))
            f.write(f"{key},{key_mse}\n")
        for dim, value in enumerate(mse_dim):
            f.write(f"dim_{dim},{value}\n")

    print(f"Results saved to {os.path.abspath(args.output_dir)}/")


def main():
    parser = argparse.ArgumentParser(formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument("--model_path", required=True, help="Path to model checkpoint dir")
    parser.add_argument("--dataset_path", required=True, help="Root of ManipArena EE dataset")
    parser.add_argument("--device", default="cuda:0")
    parser.add_argument("--prompt", default="put the object in the target location")
    parser.add_argument("--use_dataset_prompt", action="store_true", help="Read task text from parquet")
    parser.add_argument("--num_samples", type=int, default=300)
    parser.add_argument("--start_idx", type=int, default=0)
    parser.add_argument("--output_dir", default="results_maniparena")
    parser.add_argument("--log_every", type=int, default=10)
    args = parser.parse_args()
    try:
        evaluate(args)
    finally:
        if dist.is_initialized():
            dist.destroy_process_group()


if __name__ == "__main__":
    main()
